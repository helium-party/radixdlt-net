...
namespace HeliumParty.RadixDLT.Primitives
{
    ...
    public struct BigDecimal : IComparable, IComparable<BigDecimal>
    {
        ...
        #region Modified / added methods

        /// <summary>
        /// Convert the <see cref="BigDecimal"/> value to a <see cref="BigInteger"/> value, ignoring 
        /// any loss of information
        /// </summary>
        /// <returns>Value as <see cref="BigInteger"/></returns>
        public BigInteger ToBigInteger()
        {
            var biDivisor = BigInteger.One;
            var biExponent = new BigInteger(10);

            // We might not be able to represent the divisor in a normal value type like long, 
            // therefore calculate it
            for (int i = 1; i < Exponent; i++)
                biDivisor = BigInteger.Multiply(biDivisor, biExponent);

            return BigInteger.Divide(Mantissa, biDivisor);
        }
