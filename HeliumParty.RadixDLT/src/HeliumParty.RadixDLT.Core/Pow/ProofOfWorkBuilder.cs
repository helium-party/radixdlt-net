...
namespace HeliumParty.RadixDLT.Pow
{
    ...
    {
        public Task<ProofOfWork> BuildAsync(int magic, byte[] seed, int leading)
        {
            return new Task<ProofOfWork>(() => 
            {
                return Build(magic,seed,leading);
            });           
        }
        ...
        public ProofOfWork Build(int magic, byte[] seed, int leading)
        {
            ...
            int lead = leading / 8;
            lead = lead * 8;//losing precision

            for (int i = 0; i < lead; i++)
                targetBitSet.Set(i, false);

            for (int i = lead + (8 - leading % 8); i < lead + 8; i++)
                targetBitSet.Set(i, false);
